...
export default function AppliedJobDataTable() {
    ...
    const columns = [
        ...
        {
            name: 'compensation ',
            selector: row => '$' + row?.job?.compensation,
        },
        {
            name: 'Status',
        ...
        {
            name: 'Action',
            cell: row => <button onClick={() => router.push(`/frontend/jobDetails/${row?.job?._id}`)} className='md:px-2 md:py-2 px-1 py-1 text-xs text-red-600 hover:text-white my-2 hover:bg-red-600 border border-red-600   rounded transition-all duration-700  '>view Detail</button>
        },
    ];
    ...
    return (
        <>
                        ...
                        <DataTable
                            ...
                            persistTableHead
                            subHeaderComponent={
                                <input className='w-60  py-2 px-2  outline-none  border-b-2 border-red-600' type={"search"}
                                    value={search}
